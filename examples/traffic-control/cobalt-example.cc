...
int main (int argc, char *argv[])
{
  ...
  std::string  scenario = "Mix_Traffic";

  CommandLine cmd;
  cmd.AddValue ("queue_disc_type", "Queue disc type for gateway by defalut is COBALT (e.g. ns3::CobaltQueueDisc)", queue_disc_type);
  cmd.AddValue ("scenario", "scenario for simulation by defalut is Mix_Traffic (e.g. Mix_Traffic,Light_Traffic,Heavy_Traffic)", scenario);
  cmd.Parse (argc,argv);
  if (scenario != "Heavy_Traffic" && scenario != "Light_Traffic")
    {
      scenario = "Mix_Traffic";
    }
  dir = scenario + "/";
  ...
  if (scenario == "Light_Traffic")
    {
      ...
      Simulator::Schedule (Seconds (0.1), &cwnd,5);
    }
  else if (scenario == "Heavy_Traffic")
    {
      NodeContainer source;
      source.Create (50);
      ...
      Simulator::Schedule (Seconds (0.1), &cwnd,50);
    }
  else
    {
      NodeContainer source;
